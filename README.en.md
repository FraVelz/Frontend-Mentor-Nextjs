...
| `src/features/{slug}/` | Per-challenge UI: components, `page.tsx`, and **raster images and SVG** under **`…/images/`** (via `import`, not under `public/`) |
...
| `docs/challenges/{slug}/` | Challenge docs copy (template readme, `style-guide`, `design/`, preview) |
...
4. **After phase A** (or once everything needed is under `docs/challenges/`, under `src/features/{folder_name}/` including **`images/`** for graphics, and under `public/{folder_name}/` **only** when needed e.g. for fonts with fixed URLs), **move the ZIP folder out of the root** to `backups/{folder_name}/` so the tree does not keep duplicates. See [playbook §7](docs/IA/FM-CHALLENGE-PLAYBOOK.md#7-git-y-la-carpeta-del-zip).
